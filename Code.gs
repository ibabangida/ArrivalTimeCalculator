...
function replyArrivalTimeInfo(user_id, group_id, latitude, longitude)
{
  ...
  {
    var user_disp_name = getUserName(user_id, group_id);
    ...
    if (leg.arrival_time)
    {
      ...
      {
        hour += parseInt(org_time.slice(0, 1));
        min += parseInt(org_time.slice(2, 4));
      }
      // ex. 10:11pm
      else if (org_time.length == 7)
      {
        // 12:XXam(0:00), 12:XXpm(12:00)用の特殊対応
        var hour_val = parseInt(org_time.slice(0, 2));
    ...
    {
      var duration = leg.duration.value * 1000;
      var curr_time = new Date();
